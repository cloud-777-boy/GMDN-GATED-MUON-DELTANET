...
import fast_scan
from fast_scan import fast_state_scan
...
class FastWeightMixer(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, inner: str,
                 chunk: int = 64, ns_steps: int = 3, conv_kernel: int = 4):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # ---- phase 2: fused state scan (Triton kernel on GPU, one launch
        # fwd + one bwd, state pinned across the sequence; eager fallback) ----
        rule = {"delta": 0, "muon": 1, "adam": 2}[self.inner]
        nu_c_in = nu_c if nu is not None else torch.ones_like(a_c)
        states = fast_state_scan(Uv.float(), Kw.float(), a_c, b_c, mu_c,
                                 nu_c_in, rule, self.ns_steps)

        # ---- phase 3: batched output across all chunks ----
        Sst = states.to(qn.dtype)                             # (B,H,N,dh,dh)
...
class Block(nn.Module):
    def __init__(self, d_model, n_heads, inner, chunk, ns_steps=3):
        super().__init__()
...
@torch.no_grad()
def evaluate(model, val_batches, use_amp, device, mark_steps=False):
    model.eval()
    total, n = 0.0, 0
...
def run_condition(outer: str, inner: str, args, device, results_dir: str):
    ...
    mark_steps = args.compile and args.compile_mode == "reduce-overhead" \
    ...
    for step in range(1, args.steps + 1):
        ...
        for _ in range(args.accum):
            x, y = next(data)
            if mark_steps:                     # required for CUDA-graph mode +
                torch.compiler.cudagraph_mark_step_begin()  # grad accumulation
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16,
                                enabled=use_amp):
                _, loss = model(x, y)
...
def main():
    ...
    p.add_argument("--ns-steps", type=int, default=3,
                   help="inner Newton-Schulz iterations, unified across eager "
                        "and Triton backends (outer Muon optimizer stays at 5)")
    p.add_argument("--scan", default="auto", choices=["auto", "triton", "eager"],
                   help="phase-2 scan backend (auto: Triton when available)")
    p.add_argument("--scan-precision", default="tf32", choices=["tf32", "ieee"],
                   help="tl.dot precision inside the Triton scan; tf32 matches "
                        "the TF32-enabled eager path, ieee is exact fp32")
    ...
    p.add_argument("--compile-mode", default="default",
                   ...
                   help="reduce-overhead uses CUDA graphs; safe here only "
                        "because we mark step begin before every invocation")
    ...
    fast_scan.set_backend(args.scan)
    fast_scan.set_precision(args.scan_precision)
    scan_be = ("triton" if (args.scan != "eager" and fast_scan.HAS_TRITON
                            and device.type == "cuda") else "eager")
    print(f"phase-2 scan backend: {scan_be} (requested: {args.scan})")
